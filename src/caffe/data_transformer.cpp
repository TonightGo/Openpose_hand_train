...
template<typename Dtype> void DataTransformer<Dtype>::Transform_CPM(const Datum& datum, Dtype* transformed_data, Dtype* transformed_label, int cnt) {
  ...
  generateLabelMap(transformed_label, img_aug, meta, as); // and visualize
}
...
template<typename Dtype>
void DataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta, AugmentSelection as) {
  ...
  int sigma = int(param_.sigma() * as.scale);
